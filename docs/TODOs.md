...
本文档用于跟踪 simple-cloud-web 项目启动阶段尚未完成或尚未拍板的事项。

## P0：启动阶段必须确认

- [ ] 确认第一阶段目标：本地跑通、MVP 演示、上线准备或需求架构梳理。负责人：项目经理；拍板：项目经理、架构师。
- [ ] 确认第一阶段时间窗口、里程碑和交付范围。负责人：项目经理；拍板：项目经理。
- [ ] 确认 Git 仓库、分支策略、提交规范和 PR 规范。负责人：技术负责人、DevOps/CI 工程师；拍板：架构师。
- [ ] 梳理第一版需求范围和验收标准。负责人：产品经理；拍板：项目经理。
- [ ] 明确 RBAC 权限模型，包括用户、角色、菜单、按钮权限、数据范围和动态路由规则。负责人：架构师、产品经理、后端开发、前端开发；拍板：架构师。

## 2026-06-20 代码审查待办

审查说明：当前目录 `/Users/songchao/Desktop/simple-cloud-web` 不是 Git 仓库，未找到 `.git`，因此无法判断未提交代码来源；以下事项基于当前工作区源码审查。

### P0：安全阻断

- [ ] 移除前端源码中的 RSA 私钥，并确认构建产物不再包含私钥片段。当前位置：`frontend/console/src/utils/crypto.ts`；已确认 `frontend/console/dist/assets/index-*.js` 包含同一私钥。负责人：前端开发、安全工程师；拍板：架构师。
- [ ] 轮换已暴露的 RSA 密钥对，重新生成后端 `public.pem/private.pem`，并清理历史构建产物中的泄露内容。负责人：安全工程师、后端开发、运维；拍板：架构师。

### P1：安全与配置

- [ ] 将后端 cookie `keys`、MySQL 账号密码、AES key/iv、RSA 私钥路径等敏感配置迁移到环境变量或密钥管理系统。当前位置：`backend/src/config/config.local.ts`、`backend/src/config/keys/private.pem`。负责人：后端开发、安全工程师、运维；拍板：架构师。
- [ ] 收紧 `backend/src/config/keys/private.pem` 文件权限，并避免源码树保存生产私钥。负责人：安全工程师、运维；拍板：架构师。
- [ ] 补充根目录或后端 `.gitignore`，覆盖 `dist/`、`node_modules/`、`logs/`、`.DS_Store`、本地密钥和本地配置文件，避免初始化 Git 后误提交生成物和敏感文件。负责人：DevOps/CI 工程师、技术负责人；拍板：架构师。
- [ ] 完善生产环境配置。`backend/src/config/config.prod.ts` 当前仅配置 `keys` 和端口，缺少 datasource、Redis、JWT、RSA、AES 等运行必需配置。负责人：后端开发、运维；拍板：架构师。
...
- [ ] 为认证、退出登录、角色 CRUD、用户 CRUD、错误过滤器和加密响应失败补充后端 Jest 或前端 Vitest/Cypress 覆盖。负责人：后端开发、前端开发、测试。

### 本次审查验证记录

- 后端 `npm test -- --runInBand`：通过，3 个测试套件、3 个测试。
- 后端 `npm run build`：通过。
- 前端 `npm run test:unit -- --run`：通过，3 个测试文件、4 个测试。
- 前端 `npm run build-only`：通过，但有 chunk size 和 Rolldown pure annotation 警告。
- 前端 `npm run type-check`：失败。
